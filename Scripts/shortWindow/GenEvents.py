...
def get_event_noise(bx0, noise_frac=0, bkg_frac=0.2,ineff=True):
    # simulate true event
    if np.random.rand() <= (1 - bkg_frac):
        ...
        if (num_muon_hits==ps.NLAYERS) and (np.random.rand()<=0.2) and (ineff == True):
...
def generate_noisy_evts(num_events, bx0=500, noise_frac=0.1, bkg_frac=0.5,ineff=True):
    ...
    max_n_hit = 0
    for ev_id in tqdm.tqdm(range(num_events)):
        muon_hits, pattern, num_muon_hits, num_hits = get_event_noise(bx0, noise_frac=noise_frac, bkg_frac=bkg_frac,ineff=ineff)
